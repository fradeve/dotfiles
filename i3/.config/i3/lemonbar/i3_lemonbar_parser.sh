...
while read -r line ; do
  case $line in
    SYS*)
      ...
      date="%{F${color_sec_b2}}${sep_l_left}%{F${color_icon} B${color_sec_b1}} %{T2}${icon_clock}%{F- T1} ${date}"
      # time
    ...
    TW*)
      ...
      if [ "${line#???}" == "TW" ]; then
        ...
        tw="%{F${color_sec_b2}}${sep_left}%{B${color_sec_b2}}%{F${color_sec_b1}}${sep_left}%{F${color_icon} B${color_sec_b1}} %{T2}${icon_contact}%{F${color_fore} T1} ${line#???}  "
      fi
      ;;
    GMA*)
      # Gmail
    ...
    MPD*)
      ...
      if [ -z "${line#???}" ]; then
        mpd="";
      elif [ "${mpd_arr[0]}" == "mpd" ]; then
        mpd="";
      else
        mpd="%{F${color_sec_b2}}${sep_left}%{B${color_sec_b2}}%{F${color_sec_b1}}${sep_left}%{F${color_icon} B${color_sec_b1}} %{T2}${icon_music}%{F${color_fore} T1} ${line#???}"
      fi
      ;;
    WSP*)
      # I3 Workspaces
  ...
  # And finally, output
  printf "%s\n" "%{l}${wsp}${title} %{r}${tw}${mpd}${stab}${cpu}${stab}${mem}${stab}${diskr}${stab}${diskh}${stab}${wland}${stab}${wlanu}${stab}${vol}${stab}${date}${stab}${time}"
done
